...
export function generateGaragePremiumMockupHtml(p: GaragePremiumProspect): string {
  ...
  // WhatsApp : utilise le tel comme numéro WhatsApp (FR +33 implicite)
  const whatsappNumber = phoneDigits.replace(/^(?:0|33)/, "33").replace(/^33/, "+33");
  const whatsappUrl = phoneDigits ? `https://wa.me/${whatsappNumber.replace(/[^\d]/g, "")}?text=${encodeURIComponent(`Bonjour, je suis intéressé par un véhicule chez ${p.name}.`)}` : "";
  ...
  <div class="max-w-7xl mx-auto relative z-10">
    <div class="grid lg:grid-cols-12 gap-12 items-center">
      <div class="lg:col-span-7 fade-up">
        ...
        <!-- Trust badges : financement + reprise + garantie + livraison -->
        <div class="flex flex-wrap gap-2 mb-6">
          <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider" style="background: ${primary}22; color: ${primary}; border: 1px solid ${primary}44;">
            <span class="material-symbols-outlined text-sm">paid</span>Financement
          </span>
          <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider" style="background: ${primary}22; color: ${primary}; border: 1px solid ${primary}44;">
            <span class="material-symbols-outlined text-sm">swap_horiz</span>Reprise possible
          </span>
          <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider" style="background: ${primary}22; color: ${primary}; border: 1px solid ${primary}44;">
            <span class="material-symbols-outlined text-sm">shield</span>Garantie incluse
          </span>
          <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider" style="background: ${primary}22; color: ${primary}; border: 1px solid ${primary}44;">
            <span class="material-symbols-outlined text-sm">local_shipping</span>Livraison France
          </span>
        </div>
        ...
        <div class="flex flex-wrap gap-4 fade-up fade-up-2">
          ...
          ${whatsappUrl ? `<a href="${whatsappUrl}" target="_blank" rel="noopener" class="btn-ghost px-8 py-4 rounded-full font-bold uppercase tracking-wider text-base inline-flex items-center gap-2" style="border-color: #25D366; color: #25D366;">
...
<section id="contact" class="py-24 px-6 border-t border-white/10">
  ...
  <div class="max-w-7xl mx-auto mt-12 rounded-3xl overflow-hidden border border-white/10" style="height: 400px;">
    <iframe src="${mapsEmbedUrl}" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade" title="Localisation ${name}"></iframe>
  </div>
  <div class="max-w-7xl mx-auto mt-6 text-center">
    <a href="${mapsLinkUrl}" target="_blank" rel="noopener" class="btn-ghost px-6 py-3 rounded-full font-bold uppercase tracking-wider text-sm inline-flex items-center gap-2">
      <span class="material-symbols-outlined">directions</span>Itinéraire vers ${name}
    </a>
  </div>
</section>
...
${phoneDigits ? `<a href="tel:${phoneDigits}" class="md:hidden fixed bottom-6 right-6 z-50 w-16 h-16 rounded-full btn-glow flex items-center justify-center" aria-label="Appeler"><span class="material-symbols-outlined text-2xl">call</span></a>` : ""}
${whatsappUrl ? `<a href="${whatsappUrl}" target="_blank" rel="noopener" class="md:hidden fixed bottom-6 right-24 z-50 w-16 h-16 rounded-full flex items-center justify-center shadow-2xl" style="background: #25D366; box-shadow: 0 10px 30px rgba(37, 211, 102, 0.5);" aria-label="WhatsApp">
  <svg viewBox="0 0 24 24" width="28" height="28" fill="#ffffff"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413z"/></svg>
</a>` : ""}
